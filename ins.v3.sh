...
# کپی فایل‌های JSON از پوشه موجود در همان دایرکتوری
echo "coping JSON files"
if [ -d "$DATA_DIR" ]; then
    ...
    echo "training_data directory not found, make sure you habe JSON files there"
    echo "1. this directory ($PWD)"
    echo "2. up-er directory ($PWD/..)"
    exit 1
fi
